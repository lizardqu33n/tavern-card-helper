...
/**
 * World overview analysis prompt (Step 3).
 * AI analyzes character info + creator's overview, plans the world,
 * and generates a concise world overview + suggested world book entries.
 */
export const WORLD_OVERVIEW_PROMPT = (
  cardName: string,
  characterSummaries: string,
  creatorOverview: string,
) => ({
  system: `你是一位资深的世界观设计师。你的任务是根据角色信息和创作者的概述，规划一个完整、自洽、丰富的世界观。

工作流程：
1. 分析已有角色信息，提取世界观线索（时代背景、社会结构、力量体系、地理环境等）
2. 根据创作者的概述，补全世界观的空白维度
3. 确保世界观与角色设定不冲突
4. 输出一份详细的世界观概述 + 至少 10-15 条世界书条目规划

输出规则：
- 世界观概述要详细丰富，600-1000字
- 使用键值对和列表格式
- 建议条目至少 10-15 条，覆盖所有维度
- 每条条目的 summary 要详细丰富，至少 3-5 句话，包含具体细节
- 全文简体中文
...
卡片名称：${cardName}
角色信息：
${characterSummaries || '(暂无角色信息)'}

创作者的世界观概述：
${creatorOverview || '(创作者未提供概述，请根据角色信息自由规划)'}

返回 JSON 对象：
{
  "worldOverview": "世界观概述（600-1000字，键值对+列表格式）。包括：\\n时代背景：...\\n社会结构：...\\n力量体系/核心规则：...\\n地理环境：...\\n重要势力/组织：...\\n核心冲突/事件：...",
  "suggestedEntries": [
    {
      "name": "条目名称",
      "type": "背景/地点/势力/事件/物品/规则",
      "keys": ["关键词1", "关键词2", "关键词3"],
      "summary": "条目内容摘要（至少 3-5 句话，包含具体细节、场景描述、因果关系等。不要只写一句话，要写得详细丰富）",
      "priority": "high/medium/low"
    }
  ],
  "worldDimensions": ["已覆盖的维度", "建议补充的维度"]
}

重要：
- suggestedEntries 数组至少包含 10 条条目
- 每条 summary 至少 3-5 句话，包含具体细节
- 覆盖所有维度：背景历史、地点场景、势力组织、人物关系、物品道具、规则体系、事件传说等

请只输出 JSON 对象。`,
});

/**
 * World overview expansion prompt.
 * AI expands the creator's world overview with more detail.
 */
export const WORLD_OVERVIEW_EXPAND_PROMPT = (
  cardName: string,
  characterSummaries: string,
  ...
  const nsfwBlock = nsfw
    ? `\n\n## NSFW 内容指令\n本角色卡允许生成成人内容（NSFW）。在扩展世界观时：\n- 可以包含成人向的场景、关系、物品描述\n- 可以包含成人向的背景设定和事件\n- 写作风格应当自然融入世界观，不要刻意强调或过度描写`
    : `\n\n## 内容安全指令\n本角色卡不允许生成成人内容（NSFW）。在扩展世界观时：\n- 禁止包含任何成人向、性暗示或色情内容\n- 场景和关系描述应当符合全年龄标准\n- 如果世界观中可能涉及敏感内容，请以隐晦、含蓄的方式处理或直接跳过`;

  return {
    system: `你是一位资深的世界观设计师。你的任务是扩展现有的世界观概述，补充更多细节和维度。

扩展规则：
- 保留原有内容，不删改已有设定
- 补充空白维度（历史事件、文化习俗、经济体系、日常细节等）
- 确保扩展内容与角色设定一致
- 使用键值对和列表格式
- 扩展后总字数 800-1500 字
- 全文简体中文${nsfwBlock}

请只输出扩展后的世界观概述文本，不要加标题、JSON 或其他格式。`,
    user: `扩展以下世界观概述：

卡片名称：${cardName}
角色信息：${characterSummaries || '(暂无)'}

当前世界观概述：
${currentOverview}
${userRequirement ? `\n创作者的额外要求：${userRequirement}` : ''}

请扩展以上世界观，补充更多细节。只输出扩展后的文本。`,
  };
};
